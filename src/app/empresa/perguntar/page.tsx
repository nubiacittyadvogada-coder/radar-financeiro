...
export default function EmpresaPerguntarPage() {
  ...
  async function carregarHistorico(t: string) {
    const res = await fetch('/api/v2/empresa/perguntar', { headers: { Authorization: `Bearer ${t}` } })
    if (res.ok) {
      const data = await res.json()
      const flat: Msg[] = []
      data.slice().reverse().forEach((c: any) => {
        flat.push({ role: 'user', content: c.pergunta, ts: c.criadoEm })
  ...
  async function enviar(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/v2/empresa/perguntar', {
        method: 'POST',
  ...
  const sugestoes = [
    ...
    'Quais são os maiores custos da empresa?',
    'Estou conseguindo pagar os sócios de forma sustentável?',
    'Qual a tendência do meu lucro nos últimos meses?',
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <header className="bg-white border-b px-6 py-4 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Conselheiro IA</h1>
          <p className="text-sm text-gray-500">Pergunte sobre suas finanças empresariais</p>
        </div>
        <select
          value={`${mes}-${ano}`}
      ...
      <div className="flex-1 overflow-auto px-4 py-6 max-w-3xl mx-auto w-full">
        {msgs.length === 0 && !loading && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 mb-6">Pergunte qualquer coisa sobre a saúde financeira da sua empresa.</p>
